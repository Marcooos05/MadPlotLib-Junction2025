...
- Offline-first capable (progress saved locally + Google Cloud Firestore sync)
...
- Jamstack + Workflow Automation Hybrid
  - Frontend: React with Vite compiled to static assets → Vercel
  - Backend: n8n.cloud (visual workflow engine) as BFF – Backend For Frontend)
  - AI orchestration layer: n8n handles Gemini 2.0 Flash + ElevenLabs + Google Cloud Storage
  - Data layer: Google Cloud Firestore + Google Cloud Storage (via Google Cloud SDK in browser)

## State Management
- React built-in hooks (useState, useEffect) for local state
...
## Data Flow
1. User → Vercel static site (React app)
2. Onboarding answers → Google Cloud Firestore document `users/{uid}/profile`
3. Story selection → n8n webhook `/start-story` → returns story template
4. MadLib answers submitted → n8n webhook `/generate-comic`
   → parallel: Gemini sequential image gen → ElevenLabs audio → Google Cloud Storage URLs
   → webhook response with {images[], audioUrl}
5. What-If & quiz → client-side (pre-cached or same webhook with modified prompt)
6. Victory → Google Cloud Firestore update badges + optional share image upload

## Technical Stack
| Layer          | Technology                                      | Reason                                                                 |
|----------------|-------------------------------------------------|------------------------------------------------------------------------|
| Frontend       | React 18 with Vite                              | Fast UI prototyping, component-based, perfect for mobile-first design |
...
| Database       | Google Cloud Firestore in Native mode                 | Real-time sync, offline support, scales to zero                         |
| Auth           | Anonymous Google Cloud Identity (uid only)              | No friction for students                                               |

## Authentication Process
- Anonymous authentication via Google Cloud Identity on app load
- UID generated client-side → used for Google Cloud Firestore doc paths
...
## Route Design (React Router)
```
/                     → OnboardingView (redirects to onboarding if no profile)
...
All routes client-side rendered for instant navigation.
...
## Database Design ERD (Google Cloud Firestore NoSQL)
